...
export class WorkerManagementService {
  ...
  private HEALTHCHECK_INTERVAL = parseInt(
    process.env.WS_HEALTHCHECK_INTERVAL || "30000",
    10
  ); // 30 seconds
  private HEALTHCHECK_TIMEOUT = parseInt(
    process.env.WS_HEALTHCHECK_TIMEOUT || "2000",
    10
  ); // 2 seconds
  private WAIT_SHUTDOWN_TIME = parseInt(
    process.env.WS_WAIT_SHUTDOWN_TIME || "15000",
    10
  ); // 15 seconds
  private MAX_SHUTDOWN_ATTEMPTS = parseInt(
    process.env.WS_MAX_SHUTDOWN_ATTEMPTS || "3",
    10
  );
  private MAX_HEALTHCHECK_MISSED = parseInt(
    process.env.WS_MAX_HEALTHCHECK_MISSED || "9",
    10
  );
  private MAX_EXECUTION_TIME = parseInt(
    process.env.WS_MAX_EXECUTION_TIME || (15 * 60 * 1000).toString(),
    10
  ); // 15 minutes
  private temporalNamespace =
    process.env.TEMPORAL_NAMESPACE || "ob1-temporal-namespace";
  ...
  constructor(
    ...
    this.WS_WORKER_COUNT = parseInt(process.env.WS_WORKER_COUNT || "3", 10);
  ...
  public async initializeWorkers(): Promise<void> {
    ...
    for (let i = 0; i < this.WS_WORKER_COUNT; i++) {
      const success = await this.dynamicLoadingService.loadDefaultWorker(
        `worker${i}`
      );
      if (success) {
  ...
  public async initializeWorkflowClient(): Promise<void> {
    const connectionWMS = await getTemporalConnection(
      "Worker Management Service"
    );
    if (!connectionWMS) {
      logger.error(
        "Failed to establish Temporal connection for Worker Management Service. Exiting."
      );
      process.exit(1);
    }

    this.workflowClientForWMS = await getWorkflowClient(
      connectionWMS,
      "Worker Management Service"
    );
  ...
  public async getAvailableAndAssignWorker(
    workflowRunId: string,
    workflowType: string,
    temporalWorkflowId: string
  ): Promise<
    | { workerId: number; status: boolean; loaded: boolean }
    | { workerId: null; status: boolean; loaded: boolean }
  > {
    ...
    for (const [workerId, worker] of this.workers.entries()) {
      if (worker.isAvailable && worker.workflowType === workflowType) {
        ...
        logger.log(
          `WORKER ${workerId} is available and already loaded with same workflow type ${workflowType} hence skipping reloading.`
        );
        this.redisService.storeWorkerMap(this.workers);
    ...
    for (const [workerId, worker] of this.workers.entries()) {
      if (worker.isAvailable) {
        const loaded = await this.dynamicLoadingService.loadWorker(
          workerId,
          workflowType,
          temporalWorkflowId
        );
        if (loaded) {
          ...
          // logger.log(`Returning workerId: ${workerId}`);
          logger.log(
            `Assigning workflow type ${workflowType} to worker ${workerId}`
          );
          this.redisService.storeWorkerMap(this.workers);
      ...
      for (let i = 0; i < this.WS_WORKER_COUNT; i++) {
        ...
        if (!worker.isAvailable) {
          ...
          try {
            ...
            const listWorkflows =
              await this.workflowClientForWMS?.workflowService.listWorkflowExecutions(
                {
                  namespace,
            ...
            const workflowStillRunning =
              listWorkflows?.executions && listWorkflows.executions.length > 0;

            if (!workflowStillRunning) {
              worker.isAvailable = true;
              worker.workflowRunId = null;
              worker.workerReportedState =
                "ASSIGNED-TO-PREVIOUSLY-COMPLETED-WORKFLOWTYPE";
              // worker.healthChecksMissed = 0;
            ...
            if (
              worker.timeWhenAssigned != 0 &&
              currentTime - worker.timeWhenAssigned > this.MAX_EXECUTION_TIME
            ) {
              logger.log(`Current time: ${currentTime}`);
              logger.log(
                `Worker time when assigned: ${worker.timeWhenAssigned}`
              );
              logger.log(
                `Execution time: ${currentTime - worker?.timeWhenAssigned}`
              );
              logger.log(
                `WORKER ${i} is not running or has been running for more than ${this.MAX_EXECUTION_TIME}ms. Attempting shutdown...`
              );

              const shutdownSuccessful = await this.attemptWorkerShutdown(
                i,
                this.WAIT_SHUTDOWN_TIME
              );

              if (!shutdownSuccessful) {
                worker.unsuccesfulShutDownAttempts!++;
                logger.warn(
                  `Shutdown attempt failed for worker${i}. Attempt ${worker.unsuccesfulShutDownAttempts}`
                );

                if (
                  worker.unsuccesfulShutDownAttempts! >=
                  this.MAX_SHUTDOWN_ATTEMPTS
                ) {
                  logger.error(
                    `Max shutdown attempts reached for worker${i}. Forcing reset.`
                  );
                  await this.dynamicLoadingService.loadDefaultWorker(
                    `worker${i}`
                  );
                  worker.unsuccesfulShutDownAttempts = 0;
                  worker.isAvailable = true;
            ...
            const errorMessage =
              error instanceof Error
                ...
                : "An unknown error occurred";
            logger.error(
              `Error querying Temporal for worker${i}: ${errorMessage}`
            );
          }
        }
        ...
        if (worker.healthChecksMissed >= this.MAX_HEALTHCHECK_MISSED) {
          logger.error(
            `Worker${i} missed ${this.MAX_HEALTHCHECK_MISSED} health checks. Forcing reset.`
          );
  ...
  private async attemptWorkerShutdown(
    workerId: number,
    waitTime: number
  ): Promise<boolean> {
    try {
      ...
      const response = await fetch(
        `http://localhost:${5001 + workerId}/shutdown`,
        {
          method: "GET",
          signal: controller.signal,
        }
      ...
      if (response.status === 200) {
        ...
        logger.warn(
          `Shutdown returned non-200 status for worker${workerId}: ${response.status}`
        );
        return false;
      }
    } catch (error) {
