...
async function createAndRunWorker() {
    ...
    logger.log('Initializing Default Temporal worker (No Workflow OR Activity)...');
...
app.get('/health', (req: Request, res: Response) => {
    ...
    if (isShutdownState(runState)) {
        ...
        //logger.log(`HealthCheck : Worker is Healthy and in ${runState}.`);
...
app.get('/shutdown', async (req: Request, res: Response) => {
    ...
    if (!worker) {
        ...
        res.status(200).json({ status: 'already shutdown' });
        return;
    }
